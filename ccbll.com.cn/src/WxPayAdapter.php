...
class WxPayAdapter extends Client
{
    ...
    public function uploadFile($trade_no, $file_url, $oper_type = "01", $mch_no = '', $pic_type = '01', $file_type = '01')
    {
        ...
        if (isset($result['info']) || isset($result['body'])) {
            if ($result && $result['body']['rstCode'] == "0") {
                ...
                return ['err_code' => $result['info']['retCode'], "err_code_des" => ($result['body']['rstMess'] ?? $result['info']['errMsg']) . "(" . $trade_no . ")"];
            }
        } else {
     ...
     *
     * @param string $trade_no 交易流水号，随机生成, 每次请求都必须有, 建议用公共方法生成
     * @param string $jrnno 龙存管单号
     * @return array|mixed
     * @throws \GuzzleHttp\Exception\GuzzleException
     */
    public function tradeQuery($trade_no, $code = '', $jrnno = '', $trx_type = '12001', $refund_no = '')
    {
        $data = [
            'tradeNo' => $trade_no,
            'trxType' => $trx_type, //12001:B2C商城消费、12002:B2C商城消费合伙人模式、12005:用户缴费、12006:B2B商城消费、12007:B2B商城消费合伙人模式、12008:商品退款、19000:个人账户入金、19001:个人账户出金、19002:企业账户出金、19003:平台账户入金、19004:平台账户出金、21004:佣金分润、22007:平台缴费、22008:其他费用缴纳
        ];
